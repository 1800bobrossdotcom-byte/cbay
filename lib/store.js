...
const DATA_DIR = process.env.VERCEL ? path.join('/tmp', 'cbay-data') : SEED_DIR;
const CODES_FILE = path.join(DATA_DIR, 'codes.json');
const ORDERS_FILE = path.join(DATA_DIR, 'orders.json');

// Ensure data dir exists + seed from bundled data on Vercel cold start
try {
  if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
  if (process.env.VERCEL) {
    const seedCodes = path.join(SEED_DIR, 'codes.json');
    if (!fs.existsSync(CODES_FILE) && fs.existsSync(seedCodes)) {
      fs.copyFileSync(seedCodes, CODES_FILE);
    }
    const seedOrders = path.join(SEED_DIR, 'orders.json');
    if (!fs.existsSync(ORDERS_FILE) && fs.existsSync(seedOrders)) {
      fs.copyFileSync(seedOrders, ORDERS_FILE);
    }
  }
} catch (e) {
