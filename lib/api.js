...
const { sendAccessCode, sendBugReport, SUPPORT_EMAIL } = require('./mailer');
...
router.post('/order', (req, res) => {
  const { productId, coin, txHash, email } = req.body || {};
  ...
  const safeEmail = typeof email === 'string' && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim()) ? email.trim().slice(0, 254) : null;
...
router.post('/bug', bugRateLimit, (req, res) => {
  const { email, subject, body, page } = req.body || {};

  const safeBody = typeof body === 'string' ? body.slice(0, 2000) : '';
  const safeSubject = typeof subject === 'string' ? subject.slice(0, 200) : 'Bug report';
  const safePage = typeof page === 'string' ? page.slice(0, 100) : '';
  const safeEmail = typeof email === 'string' && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())
    ? email.trim().slice(0, 254) : null;

  if (!safeBody) return res.status(400).json({ error: 'description required' });
  ...
  sendBugReport({
    ...
    res.json({ submitted: true, emailed: result.sent });
  }).catch(() => {
    res.json({ submitted: true, emailed: false });
  });
});

// ── POST /api/activate — native app activation ──
// Called by Unity/Godot/TD apps to verify + redeem in one step
router.post('/activate', codeRateLimit, (req, res) => {
  const { code, deviceId } = req.body || {};
  if (!code || typeof code !== 'string') {
    return res.status(400).json({ error: 'missing code' });
  }
  ...
  const clean = code.trim().toUpperCase();
  const safeDeviceId = deviceId.slice(0, 128);

  // Verify first
  const info = verifyCode(clean);
  if (!info) {
    recordFailure(ip);
    return res.json({ activated: false, error: 'invalid or unregistered code' });
  }

  // Redeem (or re-verify same device)
  const result = redeemCode(clean, safeDeviceId);
  if (result.error) {
    recordFailure(ip);
...
// ── GET /api/support — return support info ──
router.get('/support', (req, res) => {
  res.json({
    email: SUPPORT_EMAIL,
    bugs: 'https://github.com/1800bobrossdotcom-byte/cbay/issues',
  });
});
